                ...
                <div className={`relative flex items-center justify-between px-3 lg:px-5 ${isScrolled ? 'max-w-7.5xl mx-auto' : ''}`}>
                    ...
                    <nav className="static hidden lg:flex items-center gap-1">
                        {navLinks.map((link) => (
                            <div
                                key={link.label}
                                className={`${link.isDropdown ? 'static' : 'relative'} group px-3 py-2`}
                                onMouseEnter={() => link.isDropdown && setHoveredLink(link.label)}
                                        ...
                                        <AnimatePresence>
                                            {hoveredLink === link.label && (
                                                <motion.div
                                                    ...
                                                    className="absolute top-full left-0 right-0 pt-6 z-50 gpu backface-hidden"
                                                >
                                                    <div className="mx-auto w-[900px] bg-white rounded-3xl shadow-[0_30px_60px_rgba(0,0,0,0.12)] border border-gray-100 p-6 overflow-hidden relative group/menu pointer-events-auto">
